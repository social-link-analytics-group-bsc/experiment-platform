...
def prep(df_raw):
    ...
    # merge news info
    df_raw = pd.merge(df_raw, notis_df, how='left', left_on='fake_news', right_on='data_title', suffixes=[None, '_fake'])
    df_raw = df_raw.rename(columns={"news_id": "fake_news_id", "topic_1": "fake_topic_1", "topic_2": "fake_topic_2", "data_title": "fake_news_title", "min_title": "fake_news_title_min"})
    df_raw = pd.merge(df_raw, notis_df, how='left', left_on='true_news', right_on='data_title', suffixes=[None, '_true'])
    df_raw = df_raw.rename(columns={"news_id": "true_news_id", "topic_1": "true_topic_1", "topic_2": "true_topic_2", "data_title": "true_news_title", "min_title": "true_news_title_min"})
    ...
    df_raw['dm_lugar'] = df_raw.apply(
        lambda row: row['dm_provincia'] if row['dm_provincia'] != 'Fuera de España' else unicodedata.normalize('NFD', row['dm_prov_otro'].strip().title()).encode('ascii', 'ignore').decode(), axis=1)
    df_raw['dm_lugar_country'] = df_raw.apply(
        lambda row: row['dm_prov_otro'] if row['dm_provincia'] == 'Fuera de España' else 'España', axis=1)
    # TODO [CCAA]

    df_raw['dm_education'] = df_raw.apply(
        lambda row: row['dm_educacion'] if row['dm_educacion'] != 'Otro' else row['dm_edu_otro'].strip().title(),
        axis=1)
    df_raw['dm_employment'] = df_raw.apply(
        lambda row: row['dm_empleo'] if row['dm_empleo'] != 'Otro' else row['dm_empleo_otro'].strip().title(), axis=1)
    df_raw['dm_pref_religion'] = df_raw.apply(
        lambda row: row['dm_religion'] if row['dm_religion'] != 'Otro' else row['dm_rel_otro'].strip().title(), axis=1)
    ...
    return df, df_init, df_all, n, fin, ini, read_only_1, read_both, got_to_ans, vars_all, vars_demo, vars_routine
